...
Interact with Linear via `.Codex/scripts/linear-api.sh` — a direct `curl` wrapper over Linear's GraphQL API. Each command is atomic and deterministic.

## Commands

### Issues
```bash
.Codex/scripts/linear-api.sh get-issue TEAM-66
.Codex/scripts/linear-api.sh search "authentication" 5
.Codex/scripts/linear-api.sh ai-search "error handling in mobile app" 5
.Codex/scripts/linear-api.sh create-issue "Title" "Description" 3
.Codex/scripts/linear-api.sh create-issue "Title" "-" 3 <<< "Multiline desc"
.Codex/scripts/linear-api.sh update-issue TEAM-66 --title "New Title"
.Codex/scripts/linear-api.sh update-issue TEAM-66 --description "New desc"
.Codex/scripts/linear-api.sh update-issue TEAM-66 --priority 2
.Codex/scripts/linear-api.sh update-status TEAM-66 "In Progress"
.Codex/scripts/linear-api.sh add-comment TEAM-66 "Comment body"
.Codex/scripts/linear-api.sh add-comment TEAM-66 "-" <<'EOF'
Multiline comment here
EOF
.Codex/scripts/linear-api.sh list-comments TEAM-66
```

### Labels, Assignment & Relations
```bash
.Codex/scripts/linear-api.sh add-label TEAM-66 "Bug"
.Codex/scripts/linear-api.sh remove-label TEAM-66 "Bug"
.Codex/scripts/linear-api.sh assign TEAM-66 "Alexander Basis"
.Codex/scripts/linear-api.sh add-relation TEAM-66 TEAM-67 "blocks"
.Codex/scripts/linear-api.sh link-pr TEAM-66 "https://github.com/org/repo/pull/123"
```

### Listings & Queries
```bash
.Codex/scripts/linear-api.sh list-issues 10
.Codex/scripts/linear-api.sh my-issues                    # Current user's active issues
.Codex/scripts/linear-api.sh my-issues 20                 # With custom limit
.Codex/scripts/linear-api.sh list-states
.Codex/scripts/linear-api.sh list-labels
.Codex/scripts/linear-api.sh list-users
```
...
```bash
.Codex/scripts/linear-api.sh update-status TEAM-66 "In Progress"
.Codex/scripts/linear-api.sh add-comment TEAM-66 "Implementation started. Branch: feature/team-66-feature-name"
```

### Submit for Review
```bash
.Codex/scripts/linear-api.sh update-status TEAM-66 "In Review"
.Codex/scripts/linear-api.sh add-comment TEAM-66 "-" <<'EOF'
...
### Task Done
```bash
.Codex/scripts/linear-api.sh update-status TEAM-66 "Done"
.Codex/scripts/linear-api.sh add-comment TEAM-66 "Task completed and PR merged. SHA: abc123"
```
