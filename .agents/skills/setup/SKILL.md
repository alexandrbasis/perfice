...
name: setup
description: >-
  Configure workflow for your codebase. Run after cloning .Codex/ into your repo.
  Use when asked to 'setup', 'configure workflow', 'initialize .Codex', 'setup wizard',
...
- First time after cloning `.Codex/` into a new repo
...
- Check for existing .Codex/ or .cursor/ configuration
...
- Existing .Codex/: yes/no (if yes, list what's configured)
```
...
Scan ALL `.md` files under `.Codex/skills/` and `.Codex/agents/` for remaining `{{PLACEHOLDER}}` patterns. For each file with placeholders:
...
Scan `.sh` and `.py` files in `.Codex/hooks/` for `{{PLACEHOLDER}}` patterns and replace with confirmed values. Hook scripts use `{{VARIABLE}}` syntax — Python hooks use Python literal syntax (lists, tuples), shell hooks use plain strings.
...
Wire every configured hook into `.Codex/settings.json`. Group by event and matcher:
...
To disable a skill: rename `SKILL.md` to `SKILL.md.disabled` in the skill's directory. This prevents Codex from loading it while preserving the file for re-enabling later.
...
1. Run: `grep -r '{{' .Codex/skills/ .Codex/agents/ .Codex/hooks/ --include='*.md' --include='*.sh' --include='*.py' | head -30`
...
- Edit .Codex/skills/coding-conventions/SKILL.md to fine-tune conventions
```

## Re-running Setup

If the user runs `/setup` again on an already-configured repo:
1. Grep for `{{` in `.Codex/` — if none found, the workflow is already configured
