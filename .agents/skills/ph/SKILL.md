...
Capture the current implementation state into a cold-start brief (`HANDOFF.md`) so a fresh Codex session can resume work without re-exploring the codebase. This is critical when context windows run long or when work spans multiple sessions.
