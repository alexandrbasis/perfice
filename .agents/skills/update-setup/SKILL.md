...
name: update-setup
description: >-
  Pull upstream workflow changes from claudops into your local .Codex/ folder.
  ...
  Use when asked to 'update setup', 'pull workflow changes', 'sync Codex config',
...
Sync the local `.Codex/` folder with the latest changes from the upstream claudops repository. Shows a categorized changelog, warns about conflicts, and lets the user cherry-pick what to update.

## Core Principle — Upstream-Only Scope

**This skill only tracks files that exist in the upstream claudops repo.** Users may have their own custom skills, agents, hooks, and config files in `.Codex/` — these are NEVER touched, modified, flagged, or removed. The comparison is strictly: "for each file that upstream ships, what's the local state?"
...
- **Local path**: `.Codex/` in the current working directory
...
UPSTREAM: /tmp/claudops-upstream-sync/.Codex/
LOCAL:    {CWD}/.Codex/
...
1. List ALL files in upstream .Codex/ (using Glob "**/*" under the upstream path)

2. For EACH upstream file, determine its status:

   a. **Skip entirely** — don't process these:
      - .Codex/settings.json and .Codex/settings.local.json (always local config)
      - .Codex/hooks/logs/* (runtime state)
      - Any .gitkeep files
      - AGENTS.md (project-level config, always local)
...
```bash
mkdir -p .Codex/$(dirname {path}) && cp /tmp/claudops-upstream-sync/.Codex/{path} .Codex/{path}
```

**Modified files — replace mode**: Copy upstream over local:
```bash
cp /tmp/claudops-upstream-sync/.Codex/{path} .Codex/{path}
```
...
```bash
cp /tmp/claudops-upstream-sync/.Codex/{path} .Codex/{path}.disabled
```
...
1. **Placeholder scan**: Check if any newly added or merged files contain unfilled `{{PLACEHOLDER}}` patterns:
   ```bash
   grep -rl '{{' .Codex/skills/ .Codex/agents/ .Codex/hooks/ --include='*.md' --include='*.py' --include='*.sh' 2>/dev/null | head -20
   ```
...
2. **New hooks check**: If any new hook files were added under `.Codex/hooks/`:
...
- **Local .Codex/ doesn't exist**: Tell the user to clone the workflow repo first, or run `/setup` from scratch.
